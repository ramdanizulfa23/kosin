...
<div class="relative flex flex-col w-full min-h-screen z-10 overflow-x-hidden max-w-screen-md mx-auto">
    ...
    <div id="TopNav" class="relative flex items-center justify-between px-5 mt-10 md:mt-16 w-full">
        ...
        <p class="font-bold text-black dark:text-white text-sm md:text-lg">Browse Kosin</p>
    ...
    <div id="Header" class="relative flex items-center justify-between gap-2 px-5 mt-6 w-full">
        <div class="flex flex-col gap-[6px]">
            <h1 class="font-bold text-2xl md:text-[32px] md:leading-[48px] text-black dark:text-white">Kos in {{ $city->name }}</h1>
            <p class="text-sm md:text-base text-gray-600 dark:text-gray-400">Tersedia {{ $boardingHouses->count() }} Kos in {{ $city->name }}</p>
        </div>
