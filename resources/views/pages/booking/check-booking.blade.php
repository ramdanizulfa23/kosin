...
<style>
    /* Paksa background autofill jadi gelap transparan & teks putih */
    input:-webkit-autofill,
    input:-webkit-autofill:hover,
    input:-webkit-autofill:focus,
    input:-webkit-autofill:active {
        /* Warna #1e293b (Slate 800) biar nyaru sama mode gelap/glass lu */
...
<div class="relative z-10 flex flex-col min-h-screen">
    ...
    <main class="flex flex-col gap-10 pt-[60px] pb-[150px] px-6 max-w-screen-md mx-auto w-full">
        ...
        <div class="flex flex-col gap-3 text-center">
            ...
            <p class="text-sm md:text-base text-gray-600 dark:text-gray-400">Pantau status pesanan kosan lu di sini brok</p>
        </div>

        <form action="{{ route('check-booking.show') }}" method="POST"
            class="flex flex-col rounded-[45px] border border-white/60 dark:border-white/10 p-7 md:p-12 gap-8 bg-white/40 dark:bg-slate-900/40 backdrop-blur-3xl shadow-2xl transition-all">
            @csrf

            <div class="flex flex-col gap-1 ml-2">
                <h2 class="font-black text-xl md:text-2xl text-black dark:text-white">Your Information</h2>
                <p class="text-xs md:text-sm text-gray-500 dark:text-gray-400">Gunakan data yang bener pas lu daftar ya</p>
            </div>
            ...
            <div id="InputContainer" class="flex flex-col gap-6">
                ...
                <div class="flex flex-col w-full gap-2">
                    ...
                    <label class="flex items-center w-full rounded-full p-4 md:p-5 gap-4 bg-white/60 dark:bg-[#1e293b]/50 border transition-all duration-300
                        {{ $errors->has('code') ? 'border-red-500 ring-1 ring-red-500' : 'border-white/50 dark:border-white/10' }} 
                        focus-within:ring-2 focus-within:ring-[#91BF77]">
                        <img src="{{ asset('assets/images/icons/note-favorite.svg') }}" class="w-6 h-6 flex shrink-0 dark:invert opacity-60" alt="icon">
                        <input type="text" name="code" value="{{ old('code') }}" required
                            class="appearance-none outline-none w-full bg-transparent font-black text-black dark:text-white placeholder:text-gray-400 placeholder:font-normal uppercase tracking-widest"
                            placeholder="NGKBWA185928">
                    </label>
                    @error('code')
                ...
                <div class="flex flex-col w-full gap-2">
                    ...
                    <label class="flex items-center w-full rounded-full p-4 md:p-5 gap-4 bg-white/60 dark:bg-[#1e293b]/50 border transition-all duration-300
                        ...
                        focus-within:ring-2 focus-within:ring-[#91BF77]">
                        <img src="{{ asset('assets/images/icons/sms.svg') }}" class="w-6 h-6 flex shrink-0 dark:invert opacity-60" alt="icon">
                        <input type="email" name="email" required
                            ...
                            placeholder="Alamat email lu pas daftar">
                    </label>
                    @error('email')
                    <p class="text-[10px] text-red-500 ml-5">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex flex-col w-full gap-2">
                    ...
                    <label class="flex items-center w-full rounded-full p-4 md:p-5 gap-4 bg-white/60 dark:bg-[#1e293b]/50 border transition-all duration-300
                        ...
                        focus-within:ring-2 focus-within:ring-[#91BF77]">
                        <img src="{{ asset('assets/images/icons/call.svg') }}" class="w-6 h-6 flex shrink-0 dark:invert opacity-60" alt="icon">
                        <input type="tel" name="phone_number" required
                            class="appearance-none outline-none w-full bg-transparent font-bold text-black dark:text-white placeholder:text-gray-400 placeholder:font-normal"
                            placeholder="085752140xxx">
                    </label>
                    @error('phone_number')
                    <p class="text-[10px] text-red-500 ml-5">{{ $message }}</p>
                    @enderror
